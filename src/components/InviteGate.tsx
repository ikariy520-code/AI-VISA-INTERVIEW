  ...
  useEffect(() => {
    ...
    fetch('/api/auth/status', {
      ...
      .then(async response => {
        ...
        if (response.status !== 401 && typeof payload?.message === 'string') {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#f5f5f7] px-5 py-10">
      ...
      <motion.section
        ...
        className="relative w-full max-w-[430px] rounded-[30px] border border-black/[0.06] bg-white/95 px-7 py-9 shadow-[0_24px_80px_rgba(0,0,0,0.08)] backdrop-blur-xl sm:px-10 sm:py-11"
      >
        ...
        <div className="text-center">
          ...
          <h1 className="text-[30px] font-semibold tracking-[-0.04em] text-[#1d1d1f]">测试访问</h1>
          <p className="mx-auto mt-3 max-w-[310px] text-[15px] leading-6 text-[#6e6e73]">
            当前产品处于内部测试阶段，请输入邀请码后继续。
          </p>
        </div>
