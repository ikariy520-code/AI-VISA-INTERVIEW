...
5. 可先点击“检查网络”。它不会发送 Key，只检查两套服务的域名、端口和 TLS 是否可达。
6. 点击“保存并进入应用”。应用会重启内置本地服务，确认健康后再进入练习。
...
应用只监听 `127.0.0.1`，使用随机端口，不对局域网提供服务。桌面主进程会在加载页面前请求 `/api/app-health`，并对本地服务的意外退出执行最多两次自动恢复；连续失败时会提示用户重启应用。项目不包含邀请码、订单码或次数扣减系统。

## 验证边界

仓库测试会验证三家语音供应商的配置分流、长期密钥不返回渲染器、临时凭据请求格式，以及模型无关的面签策略。要确认某个账户在真实服务中可用，仍需该用户自己的有效 API Key、对应模型访问权限、余额和网络环境；这些无法用伪造凭据替代。

无需 Key 的离线事件回放、网络检查命令和发布前真实验收清单见 [`PROVIDER_TESTING.md`](PROVIDER_TESTING.md)。
