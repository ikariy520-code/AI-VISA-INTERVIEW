...
2. 模型不再生成分数和页面排版，只返回详细证据分析包：每题的证据作用、具体优点与缺口、下一步核查，六个资格维度的完整证据链与行动建议，以及整场综合判断。项目不为节省 token 压缩实质分析，完整面试使用高推理强度和充足输出空间。
...
7. 服务器原生支持 OpenAI-compatible Chat Completions 接口。其他协议只需新增传输适配器，把相同 messages 交给模型并返回同一详细证据分析包，不应改写核心判断规则。
